...
void MaxFlow::CalcMaxFlow(bool flag){
	do{
		times++; t = 0;
		if(times%10==0)
			if(!BFS())break;
...
bool MaxFlow::BFS(){
	...
	for(int top=1;top<=bot;top++){
		...
		for(int P=E[u];P;P=Er[P].next){
			v=Er[P].to;
		    if(Er[Opposite(P)].flow && H[u]+1 < H[v]){
				H[v]=H[u]+1;
	...
	//cout<<"BFS "<<H[G.S]<<endl;
	if(H[G.S]>=inf)return false;else return true;
}
bool MaxFlow::Judge(){
	int u,v,tmp=inf;
	for(int i=1;i<=t;i++){
		...
		for(int P=E[u];P;P=Er[P].next){
			v=Er[P].to;
		    if(Er[P].flow && V[v])tmp=min(tmp,H[v]+1-H[u]);
		}
	}
...
int MaxFlow::aug(int u,int lefts){
	if(u==G.T){
		maxflow++;
		return lefts;
    }
	...
	for(int P=E[u];P && ret<lefts;P=Er[P].next){
		v=Er[P].to;
	    if(Er[P].flow && V2[v] && Ti[v]!=times && H[u]-1==H[v]){
